...
export default function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ctx!.fillStyle = '#06b6d4' // Primary brand color
        ctx!.beginPath()
      ...
      for (let a = 0; a < particles.length; a++) {
        for (let b = a; b < particles.length; b++) {
          ...
          if (distance < connectionDistance) {
            ...
            ctx.strokeStyle = `rgba(6, 182, 212, ${opacityValue * 0.5})` // Brand color with opacity
            ctx.lineWidth = 1
            ctx.beginPath()
        ...
        if (distMouse < mouseRadius) {
          ...
          ctx.strokeStyle = `rgba(6, 182, 212, ${opacityValue})` // Stronger connection to mouse
          ctx.lineWidth = 1.5
          ctx.beginPath()
          ctx.moveTo(particles[a].x, particles[a].y)
